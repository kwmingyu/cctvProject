...
def resource_path(relative_path):
        """PyInstaller 실행 환경에서도 리소스 파일 경로를 제대로 잡아주는 함수"""
        try:
            base_path = sys._MEIPASS  # PyInstaller로 실행할 때의 임시 폴더 경로
        except Exception:
...
class CCTVApp(QWidget):
    ...
    def __init__(self):
        ...
        self.scaler = joblib.load(resource_path("scaler.pkl"))
        self.model.load_state_dict(torch.load(resource_path("ae_model.pt"), map_location=self.device))
        self.model.eval()
